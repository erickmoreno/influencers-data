...
#Path to data
data_path () {
    echo data-mx/$INFLUENCER/facebook
}

#full file path
output_filename () {
    echo $(data_path)/$USERNAME-facebook-$YEAR
}

scrap_facebook () {
    echo getting $INFLUENCER $YEAR facebook posts
    mkdir -p $(data_path)
    snscrape -f $FORMAT --since "$SINCE" facebook-user $USERNAME > $(output_filename).txt
}
